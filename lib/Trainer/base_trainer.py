...
class BaseTrainer(object):
    ...
    def run_eval_epoch(self, phase, epoch, data_loader, base_s, dataset):
        ...
        for iter_id, (im_id, batch) in enumerate(data_loader):
            ...
            # print(f"DEBUG: batch['input'].shape = {batch['input'].shape}")
            ...
            # print(f"DEBUG: inp_height={inp_height}, inp_width={inp_width}, c={c}, s={s}")
            ...
            if iter_id == 0:
                with torch.no_grad():
                    raw_hm = output['hm'].clone()
                    sigmoid_hm = torch.sigmoid(raw_hm)
                    print(f"DEBUG: Raw hm output range: {raw_hm.min().item():.6f} to {raw_hm.max().item():.6f}")
                    print(f"DEBUG: Sigmoid hm range: {sigmoid_hm.min().item():.6f} to {sigmoid_hm.max().item():.6f}")
                    if len(ret[1]) > 0:
                        print(f"DEBUG: Detection scores range: {ret[1][:, 4].min():.6f} to {ret[1][:, 4].max():.6f}")
                        print(f"DEBUG: Number of detections: {len(ret[1])}")
                    else:
        ...
        # 保存可视化结果
        dataset.save_visual_results(results, opt.save_results_dir, 'latest')
